...
- [Planet rung 5A orographic](planet-rung5a-orographic.md) — first step off the zonal mean BUILT 2026-07-09 (`orographic.py`, Smith–Barstad rain shadow, wakes the inert elevation seam) + **5A.2 integration BUILT 2026-07-10** (`orographic_scene.py`: tangent-plane patch + jet-sourced wind + mm/hr→cm/yr + enhancement-only biome re-map + free serialization + demo figure) — PAYOFF: the mountain finally changes the biome map (~40% re-classified); lee-DEPLETION deferred to 5A.3; plan §12.5
