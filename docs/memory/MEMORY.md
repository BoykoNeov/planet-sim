...
- [Planet rung 2.x ITCZ](planet-rung2x-itcz.md) — full-sphere EBM + energetic ITCZ BUILT 2026-06-14 (`sphere_ebm.py`): ITCZ = the energy-flux-equator, migrates toward the warm hemisphere; the sensitivity is closed-form (not emergent). **ITCZ-sensitivity "tightening" RESOLVED 2026-07-10 as a RADIATION LIMIT**: the identity `δ/AHT=−1/(2π a² NEI(0))` (Bischoff-Schneider) ⟹ NO transport tightens it (floored at −3.9 above observed −3); the moist/MSE build (`sphere_moist_ebm.py`, a SIBLING) is ALSO a negative (moves it ~10%, `D_eff`↑ & `T̄ₓₓ`↓ cancel); → [[planet-rung2-hadley-fix]] [[planet-rung25-mse-diffusion]]
...
- [Planet rung 5B seasonal](planet-rung5b-seasonal.md) — **rung 5B COMPLETE 2026-07-10**. 5B.1 seasonal zonal EBM (`seasonal.py`): heat capacity finally load-bearing (annual limit cycle) → **continentality** from the land/ocean `C` contrast (land swings ~10× the ocean at 45°); marcher + exact spectral (`n=0` harmonic *is* the annual-mean EBM); `⟨T_L⟩=⟨T_O⟩`. 5B.2 the full **2-D lat×lon map** (`seasonal_map.py`, NMS83): single field + mask-set `C(φ,λ)`, **ADI split** (reconstructs the engine's tridiagonal assembly + hand-rolled batched Thomas / periodic Sherman–Morrison — varying-C & periodicity block the engine `step()`); **continentality is now a MAP** (interior 40 K / coast 21 K / ocean 7 K at 45°); annual mean **zonally flat for any mask** = 1-D parent (contrast is *all* amplitude); plan §12.5
- [Planet rung 5A orographic](planet-rung5a-orographic.md) — first step off the zonal mean BUILT 2026-07-09 (`orographic.py`, Smith–Barstad rain shadow, wakes the inert elevation seam) + **5A.2 integration BUILT 2026-07-10** (`orographic_scene.py`: tangent-plane patch + jet-sourced wind + mm/hr→cm/yr + enhancement-only biome re-map + free serialization + demo figure) + **5A.3 lee-depletion BUILT 2026-07-10** (`orographic_depletion.py`: opt-in along-wind moisture budget draws the lee BELOW baseline = the real desert; no-refill = L→∞ limit, L≫patch; drying-ratio DR≈0.47 calibrated) — PAYOFF: the mountain changes the biome map AND casts a real rain-shadow desert (Cascades lee 90→55 cm/yr, reclassified 41%→56%); plan §12.5
...
- [Seasonal EBM source](seasonal-ebm-source.md) — Rung 5B.1 pin (`seasonal.py`, 2026-07-10): North&Coakley1979 (seasonal EBM, land/ocean thermal inertia) + NMS83 (seasons+continents = what "2-D EBM" means) + Hartmann heat capacities (`C_atmos=c_p p_s/g`, ~50 m ocean mixed layer); `C` BUILT from textbook pieces not memorized; → [[planet-rung5b-seasonal]]
- [Smith–Barstad orographic source](smith-barstad-orographic-source.md) — Rung 5A pin (`orographic.py`, 2026-07-09): Smith&Barstad2004 LT transfer function + constants (Cw≈8.3e-3, Nm=5e-3, Hw=2500, τ=1000s) + the sgn(σ) branch/σ=0/zero-pad traps + the closed-form triangle-ridge exact anchor; → [[planet-rung5a-orographic]]
