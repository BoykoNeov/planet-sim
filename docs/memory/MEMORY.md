...
- [Planet rung 5B seasonal](planet-rung5b-seasonal.md) — seasonal zonal EBM BUILT 2026-07-10 (`seasonal.py`): heat capacity finally load-bearing (annual limit cycle, not equilibrium) → **continentality** from the land/ocean `C` contrast alone (land swings ~10× the ocean at 45°); marcher (engine-reuse) + exact spectral solve (its `n=0` harmonic *is* the annual-mean EBM); `⟨T_L⟩=⟨T_O⟩` = continentality is *all* amplitude; **"2-D EBM"=lat×lon (NMS83)** so 5B.2 map is named-not-built; plan §12.5
