...
- [Planet rung 5A orographic](planet-rung5a-orographic.md) — the first step off the zonal mean toward the north star BUILT 2026-07-09 (`orographic.py`): Smith–Barstad linear orographic precip = a diagnostic 2-D rain shadow that WAKES the inert elevation seam; a TRADE not the engine leaving the zonal mean (temperature stays zonal-mean); tight anchor = closed-form triangle-ridge; 5A.2 (sphere placement + jet source + serialization + demo) deferred; plan §12.5
